...
const TabletNumberPad = ({ value, onChange, onSave, onCancel, maxDigits = 6 }) => {
  ...
  const NumberButton = ({ number, onClick }) => (
    <motion.button
      ...
      onClick={onClick}
      className="aspect-square rounded-xl bg-gradient-to-br from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white text-2xl font-bold shadow-lg active:shadow-md transition-shadow flex items-center justify-center"
    >
      {number}
    </motion.button>
  );
...
export default TabletNumberPad;

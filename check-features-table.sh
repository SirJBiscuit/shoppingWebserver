...
docker exec -it shop_postgres psql -U postgres -d shopping_app -c "SELECT feature_key, feature_name, is_enabled, min_tier FROM feature_flags ORDER BY feature_key;"

echo ""
echo "4. Checking table structure..."
docker exec -it shop_postgres psql -U postgres -d shopping_app -c "\d feature_flags"
